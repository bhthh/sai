...
async def variable(event):  # sourcery no-metrics
    ...
    if cmd == "جلب":
        ...
        for i in configs:
            if variable in i:
                _, val = i.split("= ")
                return await cat.edit("𓆩 𝗠𝗔𝗚𝗔𝗥𝗔 - 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 𓆪\n𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻" f"\n\n**⌔∮الفـار** `{variable} = {val}`")
        await cat.edit(
            "𓆩 𝗦𝗼𝘂𝗿𝗰𝗲 𝗠𝗔𝗚𝗔𝗥𝗔 - 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 𓆪\n𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻" f"\n\n**⌔∮الفـار :** -> {variable} **غيـر موجود**❌"
        )
    elif cmd == "ضع":
    ...
    if cmd == "حذف":
        ...
        with open(config, "w") as f1:
            ...
            f1.close()
        if match:
            await cat.edit(f"**- الفـار** `{variable}`  **تم حذفه بنجاح. \n\n**- يتم الان اعـادة تشغيـل بـوت المـجرة يستغـرق الامر 2-1 دقيقـه ▬▭ ...**")
        else:
            await cat.edit(
                "𓆩 𝗦𝗼𝘂𝗿𝗰𝗲 𝗠𝗔𝗚𝗔𝗥𝗔 - 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 𓆪\n𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻" f"\n\n**⌔∮الفـار :** -> {variable} **غيـر موجود**❌"
            )
...
async def _(event):
    ...
    zed = await edit_or_reply(
        ...
        f"**⌔∮ يتـم الآن اعـادة تشغيـل بـوت المـجرة فـي السيـرفـر قـد يستغـرق الامـر 2-3 دقيقـه ▬▭ ...**",
    )
    if cmd == "كلين":
        for file in exts:
